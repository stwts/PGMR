...
def l1loss(img1,img2,mask=1,eps=1e-2):
    ...
    img1 = KF.gaussian_blur2d(img1,[3,3],torch.tensor([[1,1]]))*mask_
    img2 = KF.gaussian_blur2d(img2,[3,3],torch.tensor([[1,1]]))*mask_
...
def smoothloss(disp,img=None):
    ...
    local_smooth_re = 0
    for d in smooth_d:
        local_mean = KF.gaussian_blur2d(disp,[d,d],torch.tensor([[d//6,d//6]]),border_type='replicate')
